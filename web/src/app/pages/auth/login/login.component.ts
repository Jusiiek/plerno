...
@Component({
  ...
  template: `
    <div class="flex items-center justify-center w-screen h-screen">
      <nz-card class="w-full max-w-[800px]" nzTitle="Login">
        <form nz-form [formGroup]="form">
          ...
          <div class="flex items-right ant-flex-justify-end w-full">
            <button nz-button [nzSize]="'large'" nzType="text" class="mr-5" [routerLink]="['/auth/register']">
              Create an account
            </button>
...
export class LoginComponent {
  ...
  async onSubmit() {
    if (this.form.valid) {
      ...
      if (email && password) {
        const { res, data } = await this.authService.login(email, password);
        if (res.status === 200) {
